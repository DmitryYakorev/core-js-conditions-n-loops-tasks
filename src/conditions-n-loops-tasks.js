 ...
 *  -0.1, 0, 0.2  => 0.2
 */
function getMaxNumber(a, ...rest) {
  let current = a;
  for (let i = 0; i < 2; i += 1) {
    if (rest[i] > current) current = rest[i];
  }
  return current;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < n; i += 1) {
    if (str[i] === str[str.length - 1 - i]) counts += 1;
  }
  return n === counts;
}
...
function getSpiralMatrix(size) {
  ...
  while (counts <= full) {
    for (let i = l; i <= r && counts <= full; i += 1) {
      arr[up][i] = counts;
      counts += 1;
    }
    up += 1;
    for (let i = up; i <= down && counts <= full; i += 1) {
      arr[i][r] = counts;
      counts += 1;
    }
    r -= 1;
    for (let i = r; i >= l && counts <= full; i -= 1) {
      arr[down][i] = counts;
      counts += 1;
    }
    down -= 1;
    for (let i = down; i >= up && counts <= full; i -= 1) {
      arr[i][l] = counts;
      counts += 1;
    }
    l += 1;
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < Math.floor(l / 2); i += 1) {
    for (let j = 0; j < l - 1 - 2 * i; j += 1) {
      const t = matrix[i][i + j];
      arr[i][i + j] = matrix[l - 1 - i - j][i];
      arr[l - 1 - i - j][i] = matrix[l - 1 - i][l - 1 - i - j];
      arr[l - 1 - i][l - 1 - i - j] = matrix[i + j][l - 1 - i];
      arr[i + j][l - 1 - i] = t;
    }
  }
  return arr;
...
function sortByAsc(arr) {
  function DeliveryParts(a, left, right) {
    const pivot = a[right];
    const ar = a;
    let l = left - 1;
    ...
    [ar[l + 1], ar[right]] = [ar[right], ar[l + 1]];
    return l + 1;
  }
  function quickSort(array, left, right) {
    if (left < right) {
      const pIndex = DeliveryParts(array, left, right);
